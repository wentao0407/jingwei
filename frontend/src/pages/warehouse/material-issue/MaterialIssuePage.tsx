...
import {
  ...
  pageMaterialIssues,
  type CreateMaterialIssuePayload,
  type MaterialIssueLineRecord,
  type MaterialIssueRecord,
} from '@/services/warehouse/materialIssueService';
...
export function MaterialIssuePage() {
  ...
  const [pageResult, setPageResult] = useState<PageResult<MaterialIssueRecord> | null>(null);
  const [detailOpen, setDetailOpen] = useState(false);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detail, setDetail] = useState<MaterialIssueRecord | null>(null);
  const [formOpen, setFormOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [permissions, setPermissions] = useState<string[]>(() => getAuthSession()?.permissions ?? []);

  const canCreate = permissions.includes('warehouse:material-issue:create');
  const canConfirm = permissions.includes('warehouse:material-issue:confirm');

  const loadData = useCallback(async () => {
    setLoading(true);
    setErrorMessage('');
    try {
      setPageResult(await pageMaterialIssues({ current: 1, size: 20, ...(status ? { status } : {}) }));
  ...
  async function openDetail(record: MaterialIssueRecord) {
    setDetailOpen(true);
    setDetailLoading(true);
    try { setDetail(await getMaterialIssueDetail(record.id)); }
    catch (error) { message.error(getApiErrorMessage(error)); setDetailOpen(false); }
    finally { setDetailLoading(false); }
  }

  async function handleConfirm(record: MaterialIssueRecord) {
    try {
      await confirmMaterialIssue(record.id);
      message.success('领料已确认，库存已扣减');
      await loadData();
  ...
  if (loading && !pageResult) return <LoadingState message="正在加载领料单" />;
  if (errorMessage && !pageResult) return <ErrorState message={errorMessage} onRetry={loadData} />;

  const columns: ColumnsType<MaterialIssueRecord> = [
      ...
      render: (_, record) => (
        <Space>
          ...
          {canConfirm && record.status === 'DRAFT' ? (
            <Button size="small" icon={<CheckCircleOutlined />} type="primary" onClick={() => handleConfirm(record)}>确认领料</Button>
          ) : null}
        </Space>
      ),
    },
  ...
  return (
    <div className="system-page">
      <section className="system-page-topbar">
        <div><h1>领料出库</h1><p>按生产订单领料出库，扣减原料可用库存。</p></div>
      </section>
      <ProCard className="system-page-card" bordered={false}>
        <Space className="system-page-toolbar" wrap>
          ...
          {canCreate ? <Button icon={<PlusOutlined />} type="primary" onClick={() => { form.setFieldsValue({ lines: [{}] }); setFormOpen(true); }}>新增领料单</Button> : null}
        </Space>
        {pageResult?.records.length === 0 ? <EmptyState message="暂无领料单" /> : (
          <Table rowKey="id" columns={columns} dataSource={pageResult?.records ?? []} loading={loading} pagination={false} />
        )}
      </ProCard>

      <Modal title={detail?.issueNo ?? '领料单详情'} open={detailOpen} onCancel={() => setDetailOpen(false)} footer={null} width={800} destroyOnHidden>
        <ProCard loading={detailLoading} bordered={false}>
          {detail ? (
            <Space direction="vertical" style={{ width: '100%' }}>
              <Descriptions column={2} size="small">
                <Descriptions.Item label="领料单号">{detail.issueNo}</Descriptions.Item>
              ...
              <Table<MaterialIssueLineRecord> rowKey="id" size="small" pagination={false}
                columns={[
      ...
      <Modal title="新增领料单" open={formOpen} confirmLoading={saving} onCancel={() => setFormOpen(false)} onOk={handleCreate} okText="创建领料单" cancelText="取消" width={800} destroyOnHidden>
        <Form<IssueFormValues> form={form} layout="vertical">
          <Space style={{ width: '100%' }} wrap>
            <Form.Item label="生产订单ID" name="productionOrderId" rules={[{ required: true, message: '必填' }]}><Input /></Form.Item>
            <Form.Item label="仓库ID" name="warehouseId" rules={[{ required: true, message: '必填' }]}><Input /></Form.Item>
          </Space>
          ...
          <Form.List name="lines">
            {(fields, { add, remove }) => (
              <Space direction="vertical" style={{ width: '100%' }}>
                ...
                <Button onClick={() => add({})}>新增领料行</Button>
              </Space>
            )}
          </Form.List>
        </Form>
...
interface IssueFormLineValues { materialId?: string; batchNo?: string; quantity?: number | null; unit?: string; remark?: string; }
interface IssueFormValues { productionOrderId?: string; warehouseId?: string; remark?: string; lines?: IssueFormLineValues[]; }
