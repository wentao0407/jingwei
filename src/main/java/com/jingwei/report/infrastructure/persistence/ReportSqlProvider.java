...
public class ReportSqlProvider {
    ...
    private String buildShortageSql(Map<String, Object> params) {
        ...
        sql.append("  cw.color_name AS \"colorName\", ");
        sql.append("  sz.code AS \"sizeCode\", ");
        sql.append("  (elem->>'quantity')::INTEGER AS \"demandQty\", ");
        sql.append("  COALESCE(SUM(inv.available_qty), 0) AS \"availableQty\", ");
        sql.append("  GREATEST((elem->>'quantity')::INTEGER - COALESCE(SUM(inv.available_qty), 0), 0) AS \"shortageQty\", ");
        ...
        sql.append("JOIN LATERAL jsonb_array_elements(sl.size_matrix->'sizes') AS elem ON TRUE ");
        ...
        sql.append("JOIN t_md_size sz ON sz.id = (elem->>'sizeId')::BIGINT AND sz.deleted = FALSE ");
        ...
        sql.append("  AND sku.size_id = (elem->>'sizeId')::BIGINT AND sku.deleted = FALSE ");
        ...
        sql.append("  AND (elem->>'quantity')::INTEGER > 0 ");
        appendShortageFilters(sql, params);
        sql.append("GROUP BY so.id, so.order_no, so.customer_id, cust.name, ");
        sql.append("  spu.id, spu.code, spu.name, cw.color_name, sz.code, elem->>'quantity', so.delivery_date, so.status ");
        sql.append("HAVING (elem->>'quantity')::INTEGER > COALESCE(SUM(inv.available_qty), 0) ");
        return sql.toString();
    }
