...
## 当前开发阶段
第一阶段（项目骨架 + 公共模块 + 系统管理 + 基础数据）已全部完成。
第二阶段（状态机引擎 + 审批引擎 + 订单管理 + 采购管理）已全部完成（T-16~T-28）。
第三阶段库存管理已全部完成：T-29~T-36 已完成。
第四阶段横切模块已全部完成：T-37（通知中心）、T-38（退货管理）、T-39（成本核算）、T-40~T-43（数据一致性保障+报表）。
所有预留钩子（9处）已于 2026-05-05 全部完成。
代码审查修复已完成（v1.5.1）：DataPermission 拦截器、编码规则迁移、条件评估器 mock 补全、581 测试全通过。
下一步：出入库作业 - 领料/退料、前端开发（T-44~T-51）。

## 已完成模块
- [x] 项目骨架（Spring Boot 项目初始化、包结构、公共模块）✅ 单元测试通过
- [x] 公共模块 — 统一异常处理与响应封装（T-02）✅ 单元测试通过
- [x] 公共模块 — 审计字段基类与乐观锁基类（T-03）✅ 单元测试通过
- [x] 系统管理 - 用户/角色/登录/JWT ✅ 单元测试通过
- [x] 系统管理 - RBAC 权限（菜单级+按钮级+数据权限）✅ 单元测试通过
- [x] 系统管理 - 数据权限管理（US-SYS-003，DataPermissionInterceptor 已实现，当前因 MyBatis 兼容性暂时禁用）✅ 单元测试通过
- [x] 系统管理 - 操作日志查询（US-SYS-004，含 AOP 切面 @OperateLog 自动记录）✅ 单元测试通过
- [x] 系统管理 - 系统配置管理（US-SYS-005，含启动时本地缓存+查询走缓存+修改刷新）✅ 单元测试通过
- [x] 系统管理 - 密码策略补全（SYS-TODO-1，8位+复杂度+过期检查+修改密码）✅ 单元测试通过
- [x] 基础数据 - 编码规则引擎 ✅ 单元测试通过
- [x] 基础数据 - 物料分类管理 ✅ 单元测试通过
- [x] 基础数据 - 物料主数据 ✅ 单元测试通过
- [x] 基础数据 - 尺码组与尺码管理 ✅ 单元测试通过
- [x] 基础数据 - 季节与波段管理 ✅ 单元测试通过
- [x] 基础数据 - SPU/SKU 管理 ✅ 单元测试通过
- [x] 基础数据 - 供应商/客户档案 ✅ 单元测试通过
- [x] 基础数据 - 仓库/库位档案 ✅ 单元测试通过
- [x] 订单管理 - 通用状态机引擎（T-16）✅ 单元测试通过
- [x] 订单管理 - 销售订单状态机配置（T-17，条件评估器和动作执行器使用预留钩子）✅ 单元测试通过
- [x] 订单管理 - 审批引擎（T-18，单人审批+或签，审批结果通过 Outbox 领域事件通知）✅ 单元测试通过
- [x] 订单管理 - 销售订单表结构与CRUD（T-19，SizeMatrix值对象+尺码矩阵JSONB+金额自动计算）✅ 单元测试通过
- [x] 订单管理 - 销售订单状态流转与审批集成（T-20，状态机驱动+审批引擎集成+库存预留/释放事件+变更日志自动记录）✅ 单元测试通过
- [x] 订单管理 - 销售订单变更管理与时间线（T-21，数量变更单+差异矩阵+审批流程+时间线查询+字段变更记录）✅ 单元测试通过
- [x] 订单管理 - 生产订单（T-22，生产订单表结构+CRUD+行独立状态+主表最滞后状态计算+skip_cutting标记）✅ 单元测试通过
- [x] 订单管理 - 生产订单状态机配置与流转（T-23，ProductionOrderEvent+条件评估器+动作执行器+变更日志监听器+行级别状态流转）✅ 单元测试通过
- [x] 订单管理 - 订单转化（T-24，销售订单→生产订单+同款合并+拆分生产+BOM自动关联+source多对多关联+条件评估器钩子替换）✅ 单元测试通过
- [x] 订单管理 - 退货管理（T-38，ReturnOrder/Line+三种退货类型+6状态流转+审批集成+收货质检+库存交互）✅ 单元测试通过
- [x] 采购管理 - BOM管理（T-25，BOM主表+行项目+三种消耗类型+尺码用量JSONB+版本控制+审批生效+旧版本自动淘汰）✅ 单元测试通过
- [x] 采购管理 - MRP计算引擎（T-26，六步计算流程+BOM展开+同物料合并+库存抵扣+采购建议+来源追溯+快照时间）✅ 单元测试通过
- [x] 采购管理 - 采购订单管理（T-27，采购订单CRUD+8条状态机转移+审批集成+行金额自动计算+MRP结果生成预留）✅ 单元测试通过
- [x] 采购管理 - 到货检验（T-28，ASN到货通知单+收货确认+来料检验+QcResult JSONB+三种检验结果处理+采购订单行数量回写+库存变更预留接口）✅ 单元测试通过
- [x] 库存管理 - 四类库存模型（T-29，inventory_sku/inventory_material/in_transit/operation四表+InventoryDomainService核心变更+乐观锁重试+操作流水）
- [x] 库存管理 - 库存预留与释放（T-30，InventoryAllocation+AllocationDomainService全额/部分/无库存预留+释放+出库完成+过期自动释放）
- [x] 库存管理 - 入库/出库单（T-31，InboundOrder/Line+OutboundOrder/Line+四种入库类型+四种出库类型+确认入库/出库驱动changeInventory+库存不足校验）
- [x] 库存管理 - 盘点管理（T-32，StocktakingOrder/Line+盲盘/明盘模式+差异比对+审核调整+盘盈盘亏+库位冻结/解冻+复盘阈值）
- [x] 库存管理 - 库存预警（T-33，AlertRule+InventoryAlert+低库存/超储/库龄三种预警+去重扫描+确认处理）
- [x] 出入库作业 - 收货/上架（T-34，ReceivingOrder/Line+从ASN创建收货单+确认收货+库位推荐+确认上架）
- [x] 出入库作业 - 波次/拣货/复核/发货（T-35/T-36，Wave+PickList/PickItem+波次策略+拣货确认+短拣检测+发货确认）
- [ ] 出入库作业 - 领料/退料
- [x] 通知中心（T-37，Notification+Receiver+Channel+Preference四表+站内消息+外部渠道推送+通知偏好+6个接口）✅ 单元测试通过
- [x] 成本核算（T-39，CostProductionOrder+CostMaterialIssue+领料成本记录+成品单位成本+移动加权平均法+2个查询接口）✅ 单元测试通过
- [x] 数据一致性保障 - Outbox与幂等（T-40，DomainEventOutbox+OutboxEventRelay+IdempotencyInterceptor+Redis SETNX+事件消费幂等）✅ 单元测试通过
- [x] 数据一致性保障 - 日终对账（T-41，ReconciliationDomainService+定时任务+对账异常表+操作流水汇总vs库存余额）✅ 单元测试通过
- [x] 报表 - 库存台账与出入库流水（T-42，ReportMapper+库存台账查询+矩阵视图+出入库流水+Excel导出）✅ 单元测试通过
- [x] 报表 - 库龄分析与畅滞销分析（T-43，库龄区间分组+超期预警+周转天数+畅销等级+Excel导出）✅ 单元测试通过
- [x] 预留钩子全部完成（9处：基础数据引用检查5处+状态机条件评估器4处）✅
- [ ] 前端 - 工程初始化与基础架构
- [ ] 前端 - 全局布局与登录认证
- [ ] 前端 - 通用业务组件库
- [ ] 前端 - 系统管理页面
