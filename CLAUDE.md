...
第二阶段（状态机引擎 + 审批引擎 + 订单管理 + 采购管理）已完成 T-16~T-28，仅 T-24 订单转化已于 2026-05-05 完成。
下一步：第三阶段库存管理 + 出入库作业（T-29~T-36）。
...
- [x] 订单管理 - 订单转化（T-24，销售订单→生产订单+同款合并+拆分生产+BOM自动关联+source多对多关联+条件评估器钩子替换）
- [ ] 订单管理 - 退货管理
